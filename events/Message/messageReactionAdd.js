  ...
  async execute(messageReaction, user) {
    ...
    if (a.events.messageReactionAdd === true) {
      ...
      const embed = new MessageEmbed()
        .setColor('#5cb85c')
        .setAuthor({name : `${user.tag} has added a reaction to a message.`, iconURL: user.displayAvatarURL({dynamic: true})})
        ...
        .setFooter({text : `Watcher Event • Reaction Added | Message ID: ${message.id} • Author ID: ${user.id}`})
        .setTimestamp();
      setTimeout( function() {
