    ...
    if (ignoreFetch) {
       if (message.channel.id === ignoreFetch) return
        ...
        if (fetch === true) {
            ...
            .setColor("#7289DA").setTitle("Message Deleted").setURL("https://discord.gg/EH7jKFH").setDescription(`${message.author.tag} (ID:${message.author.id}) has deleted their message sent in ${message.channel}.\n\n\`\`\`${contentValue}\`\`\``).setFooter(`ID: ${message.id}`).setTimestamp()
