  ...
  async execute(message) {
    ...
    if (a.events.messageDelete === true) {
      ...
      if (!check || !fetchedLogs) {
        ...
        embed2.setAuthor({name : `${message.author.tag}'s${text} message has been deleted.`, iconURL: message.author.displayAvatarURL({dynamic: true})});
        ...
        embed2.setFooter({text: `Watcher Event • Message Deleted | Author ID: ${message.author.id} • Message ID: ${message.id}`});
        embed2.setTimestamp();
      } else {
        if (message.author.id === check.target.id) {
          textUser = `This message was deleted by **${check.executor.tag}**.\n`;
        } else { textUser = '*May have possibly deleted their own message!*\n'; }
