  ...
  async execute(guild, user) {
    ...
    if (fetch === true) {
      ...
      const embed = new MessageEmbed()
        .setColor('#D92C2C').setTitle('Member Banned').setURL('https://discord.gg/83SAWkh').setDescription(`${user.tag} (ID:${user.id}) was banned.`).setFooter(`ID: ${user.id}`).setTimestamp();
