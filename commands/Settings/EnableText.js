...
module.exports = class extends Command {
  ...
  async execute(message) {
    if (!db.has(`guild_${message.guild.id}.logChannel`)) {
      ...
      if (value.toUpperCase() === 'ON' || value.toUpperCase() === 'enable') return db.set(`guild_${message.guild.id}.textLog`, true) && message.channel.send(`${message.author} | Logs will __now__ be text form, database updated.`);
      if (value.toUpperCase() === 'OFF' || value.toUpperCase() === 'disable') return db.set(`guild_${message.guild.id}.textLog`, false) && message.channel.send(`${message.author} | Logs will __not__ be in text form, database updated.`);
