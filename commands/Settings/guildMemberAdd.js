...
module.exports = class extends Command {
  ...
  async execute(message) {
    ...
    if (b.wb.wbID === null || b.wb.wbKey === null) {
      ...
      if (value.toUpperCase() === 'ON' || value.toUpperCase() === 'enable') return await db.update(message.guild.id, this.client.mongod, 'events', {'events.guildMemberAdd': true}) && message.channel.send(`${message.author} | Logs will __now__ include \`guildMemberAdd\`, database updated.`);
      if (value.toUpperCase() === 'OFF' || value.toUpperCase() === 'disable') return await db.update(message.guild.id, this.client.mongod, 'events', {'events.guildMemberAdd': false}) && message.channel.send(`${message.author} | Logs will __not__ include \`guildMemberAdd\`, database updated.`);
