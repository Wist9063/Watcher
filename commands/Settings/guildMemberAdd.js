...
module.exports = class extends Command {
  ...
  execute(message) {
    if (!db.has(`log-channel_${message.guild.id}`)) {
      ...
      if (value.toUpperCase() === 'ON') return db.set(`guildMemberAdd_${message.guild.id}`, { value: true }) && message.channel.send(`${message.author} | Logs will __now__ include \`guildMemberAdd\`, database updated.`).then(msg => msg.delete({timeout:10000}));
      if (value.toUpperCase() === 'OFF') return db.set(`guildMemberAdd_${message.guild.id}`, { value: false }) && message.channel.send(`${message.author} | Logs will __not__ include \`guildMemberAdd\`, database updated.`).then(msg => msg.delete({timeout:10000}));
