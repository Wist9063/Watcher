  ...
  async execute(message) {
    await db.get(message.guild.id, this.client.mongod, 'guildSettings').then((b) => {
      if (b.wb.wbID === null || b.wb.wbKey === null) {
        ...
        if (value.toUpperCase() === 'ON' || value.toUpperCase() === 'enable') return this.client.mongod.db('watcher').collection('events').findOneAndUpdate({gID: message.guild.id}, {$set: {'events.guildBanRemove': true}}) && message.channel.send(`${message.author} | Logs will __now__ include \`guildMemberAdd\`, database updated.`);
        if (value.toUpperCase() === 'OFF' || value.toUpperCase() === 'disable') return this.client.mongod.db('watcher').collection('events').findOneAndUpdate({gID: message.guild.id}, {$set: {'events.guildBanRemove': false}}) && message.channel.send(`${message.author} | Logs will __not__ include \`guildMemberAdd\`, database updated.`);
