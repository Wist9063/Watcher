    ...
    if (b.wb.wbID === null || b.wb.wbKey === null) {
      ...
      if (value.toUpperCase() === 'ON' || value.toUpperCase() === 'ENABLE') {
        if (a.events.messageReactionAdd === true) {
          return message.reply({ content: 'The event `messageReactionAdd` is already enabled.', allowedMentions: { repliedUser: false }});
        } else {
          return await db.update(message.guild.id, this.client.mongod, 'events', {'events.messageReactionAdd': true}) && message.reply({ content: 'Logs will __now__ include `messageReactionAdd`, database updated.', allowedMentions: { repliedUser: true }});
        }
      }
      if (value.toUpperCase() === 'OFF' || value.toUpperCase() === 'DISABLE') {
        if (a.events.messageReactionAdd === false) {
          return message.reply({ content: 'The event `messageReactionAdd` is already disabled.', allowedMentions: { repliedUser: false }});
        } else {
          return await db.update(message.guild.id, this.client.mongod, 'events', {'events.messageReactionAdd': false}) && message.reply({ content: 'Logs will __not__ include `messageReactionAdd`, database updated.', allowedMentions: { repliedUser: true }});
        }
      }
