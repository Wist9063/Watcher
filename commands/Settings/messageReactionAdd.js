...
module.exports = class extends Command {
  ...
  execute(message) {
    if (!db.has(`guild_${message.guild.id}.logChannel`)) {
      ...
      if (value.toUpperCase() === 'ON') return db.set(`guild_${message.guild.id}.events.messageReactionAdd`, true) && message.channel.send(`${message.author} | Logs will __now__ include \`messageReactionAdd\`, database updated.`).then(msg => msg.delete({timeout:10000}));
      if (value.toUpperCase() === 'OFF') return db.set(`guild_${message.guild.id}.events.messageReactionAdd`, false) && message.channel.send(`${message.author} | Logs will __not__ include \`messageReactionAdd\`, database updated.`).then(msg => msg.delete({timeout:10000}));
