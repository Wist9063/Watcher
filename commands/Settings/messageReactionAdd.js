...
module.exports = class extends Command {
  ...
  execute(message) {
    if (!db.has(`log-channel_${message.guild.id}`)) {
      ...
      if (value.toUpperCase() === 'ON') return db.set(`messageReactionAdd_${message.guild.id}`, { value: true }) && message.channel.send(`${message.author} | Logs will __now__ include \`messageReactionAdd\`, database updated.`).then(msg => msg.delete({timeout:10000}));
      if (value.toUpperCase() === 'OFF') return db.set(`messageReactionAdd_${message.guild.id}`, { value: false }) && message.channel.send(`${message.author} | Logs will __not__ include \`messageReactionAdd\`, database updated.`).then(msg => msg.delete({timeout:10000}));
