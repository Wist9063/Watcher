...
module.exports = class extends Command {
  ...
  execute(message) {
    if (!db.has(`guild_${message.guild.id}.logChannel`)) {
      ...
      if (value.toUpperCase() === 'ON') return db.set(`messageReactionRemove_${message.guild.id}`, true) && message.channel.send(`${message.author} | Logs will __now__ include \`messageReactionRemove\`, database updated.`).then(msg => msg.delete({timeout:10000}));
      if (value.toUpperCase() === 'OFF') return db.set(`messageReactionRemove_${message.guild.id}`, false) && message.channel.send(`${message.author} | Logs will __not__ include \`messageReactionRemove\`, database updated.`).then(msg => msg.delete({timeout:10000}));
