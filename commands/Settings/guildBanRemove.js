    ...
    if (b.wb.wbID === null || b.wb.wbKey === null) {
      ...
      if (value.toUpperCase() === 'ON' || value.toUpperCase() === 'ENABLE') {
        if (a.events.guildBanAdd === true) {
          return message.reply({ content: 'The event `guildBanRemove` is already enabled.', allowedMentions: { repliedUser: false }});
        } else {
          return await db.update(message.guild.id, this.client.mongod, 'events', {'events.guildBanRemove': true}) && message.reply({ content: 'Logs will __now__ include `guildBanRemove`, database updated.', allowedMentions: { repliedUser: true }});
        }
      }
      if (value.toUpperCase() === 'OFF' || value.toUpperCase() === 'DISABLE') {
        if (a.events.guildBanRemove === false) {
          return message.reply({ content: 'The event `guildBanRemove` is already disabled.', allowedMentions: { repliedUser: false }});
        } else {
          return await db.update(message.guild.id, this.client.mongod, 'events', {'events.guildBanRemove': false}) && message.reply({ content: 'Logs will __not__ include `guildBanRemove`, database updated.', allowedMentions: { repliedUser: true }});
        }
      }
