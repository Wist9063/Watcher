...
module.exports = class extends Command {
  ...
  execute(message) {
    if (!db.has(`guild_${message.guild.id}.logChannel`)) {
      ...
      if (value.toUpperCase() === 'ON' || value.toUpperCase() === 'enable' || value.toUpperCase() === 'enable') return db.set(`guild_${message.guild.id}.events.guildMemberUpdate`, true) && message.channel.send(`${message.author} | Logs will __now__ include \`guildMemberUpdate\`, database updated.`).then(msg => msg.delete({timeout:10000}));
      if (value.toUpperCase() === 'OFF' || value.toUpperCase() === 'disable') return db.set(`guild_${message.guild.id}.events.guildMemberUpdate`, false) && message.channel.send(`${message.author} | Logs will __not__ include \`guildMemberUpdate\`, database updated.`).then(msg => msg.delete({timeout:10000}));
