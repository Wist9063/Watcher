...
module.exports = class extends Command {
  ...
  execute(message) {
    ...
    if (value.toUpperCase() === 'ON') return db.set(`guildMemberRemove_${message.guild.id}`, { value: true }) && message.channel.send(`${message.author} | Logs will __now__ include \`guildMemberRemove\`, database updated.`).then(msg => msg.delete({timeout:10000}));
    if (value.toUpperCase() === 'OFF') return db.set(`guildMemberRemove_${message.guild.id}`, { value: false }) && message.channel.send(`${message.author} | Logs will __not__ include \`guildMemberRemove\`, database updated.`).then(msg => msg.delete({timeout:10000}));
