...
module.exports = class extends Command {
  ...
  execute(message) {
    if (!db.has(`guild_${message.guild.id}.logChannel`)) {
      ...
      if (value.toUpperCase() === 'ON') return db.set(`guild_${message.guild.id}.events.guildMemberRemove`, true) && message.channel.send(`${message.author} | Logs will __now__ include \`guildMemberRemove\`, database updated.`).then(msg => msg.delete({timeout:10000}));
      if (value.toUpperCase() === 'OFF') return db.set(`guild_${message.guild.id}.events.guildMemberRemove`, false) && message.channel.send(`${message.author} | Logs will __not__ include \`guildMemberRemove\`, database updated.`).then(msg => msg.delete({timeout:10000}));
