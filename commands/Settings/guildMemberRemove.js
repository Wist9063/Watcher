    ...
    if (b.wb.wbID === null || b.wb.wbKey === null) {
      ...
      if (value.toUpperCase() === 'ON' || value.toUpperCase() === 'ENABLE') {
        if (a.events.guildMemberRemove === true) {
          return message.reply({ content: 'The event `guildMemberRemove` is already enabled.', allowedMentions: { repliedUser: false }});
        } else {
          return await db.update(message.guild.id, this.client.mongod, 'events', {'events.guildMemberRemove': true}) && message.reply({ content: 'Logs will __now__ include `guildMemberRemove`, database updated.', allowedMentions: { repliedUser: true }});
        }
      }
      if (value.toUpperCase() === 'OFF' || value.toUpperCase() === 'DISABLE') {
        if (a.events.guildMemberRemove === false) {
          return message.reply({ content: 'The event `guildMemberRemove` is already disabled.', allowedMentions: { repliedUser: false }});
        } else {
          return await db.update(message.guild.id, this.client.mongod, 'events', {'events.guildMemberRemove': false}) && message.reply({ content: 'Logs will __not__ include `guildMemberRemove`, database updated.', allowedMentions: { repliedUser: true }});
        }
      }
