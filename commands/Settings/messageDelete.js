    ...
    if (b.wb.wbID === null || b.wb.wbKey === null) {
      ...
      if (value.toUpperCase() === 'ON' || value.toUpperCase() === 'ENABLE') {
        if (a.events.messageDelete === true) {
          return message.reply({ content: 'The event `messageDelete` is already enabled.', allowedMentions: { repliedUser: false }});
        } else {
          return await db.update(message.guild.id, this.client.mongod, 'events', {'events.messageDelete': true}) && message.reply({ content: 'Logs will __now__ include `messageDelete`, database updated.', allowedMentions: { repliedUser: true }});
        }
      }
      if (value.toUpperCase() === 'OFF' || value.toUpperCase() === 'DISABLE') {
        if (a.events.messageDelete === false) {
          return message.reply({ content: 'The event `messageDelete` is already disabled.', allowedMentions: { repliedUser: false }});
        } else {
          return await db.update(message.guild.id, this.client.mongod, 'events', {'events.messageDelete': false}) && message.reply({ content: 'Logs will __not__ include `messageDelete`, database updated.', allowedMentions: { repliedUser: true }});
        }
      }
