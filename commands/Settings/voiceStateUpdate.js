    ...
    if (b.wb.wbID === null || b.wb.wbKey === null) {
      ...
      if (value.toUpperCase() === 'ON' || value.toUpperCase() === 'ENABLE') {
        if (a.events.voiceStateUpdate === true) {
          return message.reply({ content: 'The event `voiceStateUpdate` is already enabled.', allowedMentions: { repliedUser: false }});
        } else {
          return await db.update(message.guild.id, this.client.mongod, 'events', {'events.voiceStateUpdate': true}) && message.reply({ content: 'Logs will __now__ include `voiceStateUpdate`, database updated.', allowedMentions: { repliedUser: true }});
        }
      }
      if (value.toUpperCase() === 'OFF' || value.toUpperCase() === 'DISABLE') {
        if (a.events.voiceStateUpdate === false) {
          return message.reply({ content: 'The event `voiceStateUpdate` is already disabled.', allowedMentions: { repliedUser: false }});
        } else {
          return await db.update(message.guild.id, this.client.mongod, 'events', {'events.voiceStateUpdate': false}) && message.reply({ content: 'Logs will __not__ include `voiceStateUpdate`, database updated.', allowedMentions: { repliedUser: true }});
        }
      }
