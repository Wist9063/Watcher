    ...
    if (b.wb.wbID === null || b.wb.wbKey === null) {
      ...
      if (value.toUpperCase() === 'ON' || value.toUpperCase() === 'ENABLE') {
        if (a.events.channelDelete === true) {
          return message.reply({ content: 'The event `channelDelete` is already enabled.', allowedMentions: { repliedUser: false }});
        } else {
          return await db.update(message.guild.id, this.client.mongod, 'events', {'events.channelDelete': true}) && message.reply({ content: 'Logs will __now__ include `channelDelete`, database updated.', allowedMentions: { repliedUser: true }});
        }
      }
      if (value.toUpperCase() === 'OFF' || value.toUpperCase() === 'DISABLE') {
        if (a.events.channelDelete === false) {
          return message.reply({ content: 'The event `channelDelete` is already disabled.', allowedMentions: { repliedUser: false }});
        } else {
          return await db.update(message.guild.id, this.client.mongod, 'events', {'events.channelDelete': false}) && message.reply({ content: 'Logs will __not__ include `channelDelete`, database updated.', allowedMentions: { repliedUser: true }});
        }
      }
