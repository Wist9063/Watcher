...
class PermissionLevel {
  ...
  fetchOther(client, message) {
    ...
    if (config.owners.includes(message.mentions.users.first().id)) return [10, '<:VerifiedBotTechnician:503405757719511063> **Owner** - **Official Bot Owner**'];
    if (config.helpers.includes(message.mentions.users.first().id)) return [9, '<:VerifiedBotTechnician:503405757719511063> **Development Team** - **Verified Bot Technician**'];
    if (config.admins.includes(message.mentions.users.first().id)) return [8, '<:VerifiedBotTechnician:503405757719511063> **Global Admin** - **Verified Bot Admin**'];
    if (config.mods.includes(message.mentions.users.first().id)) return [7, '<:VerifiedBotTechnician:503405757719511063> **Global Mod** - **Verified Bot Moderator**'];
